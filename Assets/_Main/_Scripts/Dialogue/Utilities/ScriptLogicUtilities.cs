...
namespace Dialogue
{
	public static class ScriptLogicUtilities
	{
		...
		public static string EvaluateExpression(string valueString, VariableManager variableManager)
		{
			...
			valueString = ScriptVariableUtilities.ParseLogic(valueString, variableManager);

			// Split the string into operators and operands
			MatchCollection operatorMatches = CalculationOperatorRegex.Matches(valueString);
			if (operatorMatches.Count == 0) return EvaluateStringValue(valueString, variableManager);
			...
			for (int groupIndex = 0; groupIndex < OrderedOperators.Length; groupIndex++)
			{
				...
				for (int opIndex = 0; opIndex < valueOperators.Count;)
				{
					...
					valueOperands[opIndex + 1] = CalculateValue(valueOperands[opIndex], expressionOp, valueOperands[opIndex + 1], variableManager);
		...
		public static string CalculateValue(string leftOperand, string middleOperator, string rightOperand, VariableManager variableManager)
		{
			...
			rightOperand = EvaluateStringValue(rightOperand, variableManager);
		...
		public static string EvaluateStringValue(string rawValue, VariableManager variableManager)
		{
			...
			baseValue = ScriptVariableUtilities.ParseLogic(baseValue.Trim(), variableManager);
		...
		public static string RemoveStringQuotes(string stringValue)
		{
